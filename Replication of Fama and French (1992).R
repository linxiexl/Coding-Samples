#########################################
# Replication of Fama and French (1992) #
# by XIE Lin                            #
#########################################


setwd("E:/PhD Application/Replication/FF1992")
getwd()
...
CRSP <- fread("crsp.csv", header = TRUE)
head(CRSP, 3)
...
#change RET and DLRET to NA for later calculation results
...
#number of SHROUT is measured in thousands, so multiply 1000
CRSP[, SHROUT:=SHROUT*1000, ]

#calculate total market capitalization when there are more than 1 PERMNO
...
Link_table <- fread("linktable.csv", header = TRUE)
head(Link_table, 3)
...
COMPDT <- fread("compustat.csv", header = TRUE)

#COMPUSTAT annual pension
...
#merge COMPUSTAT and COMPUSTAT annual pension
...
# select columns needed in compustat_link
#compustat_link <- compustat_link[, c("DATE", "GVKEY", "fyear", "be", "LPERMNO", "LPERMCO")]

#merge compustat_link with CRSP
setkey(compustat_link, fyear, LPERMNO, LPERMCO)
#fiscal year and calander year treating same as in the paper
CRSP[, fyear:=year(date)]
CRSP[, july_jun_year := year(YearMonth) - (month(YearMonth) < 7)-1] 
#july_jun year with a lag so Jul1970 to Jun 1971 is july_jun_year fyear 1969,
#this will help in getting lagged book equity from 
#compustat table directly
setkey(CRSP, july_jun_year, PERMNO, PERMCO)

final_DT <- merge(CRSP, compustat_link, 
                  by.x = c("PERMNO", "july_jun_year", "PERMCO"), 
                  by.y = c("LPERMNO", "fyear", "LPERMCO"), 
                  all.x = TRUE, allow.cartesian = TRUE)


#############################
#Cleaning of the merged file#
#############################
...
if(nrow(unique(final_DT, by = c(
  'GVKEY', 'date'))) != nrow(
    final_DT) | nrow(unique(final_DT, by = c(
      'PERMCO', 'date'))) != nrow(final_DT)) {print(
        '1. Monthly Firm-level returns.R: There is an issue with your merge 
        between Monthly CRSP and the CRSP/Compustat linktable!')}

#select variables needed for further use
final_DT_bkp <- final_DT
final_DT = final_DT[, .(PERMNO, 
                        july_jun_year, 
                        PERMCO, date,
                        YearMonth, SHRCD, 
                        EXCHCD, DLRET, PRC, 
                        RET, SHROUT, adjRet, Mkt_Cap, lag_Mkt_Cap, GVKEY, be)]
...
# decile for Size
size_DT[month(YearMonth)==7, JulDecile := as.integer(
  cut(lag_Mkt_Cap, quantile(
    lag_Mkt_Cap[EXCHCD==1], 
    probs = 0:10/10, na.rm = TRUE)), include.lowest = TRUE), by=.(YearMonth)]
...
size_Ret <- unique(size_DT[decile>0 & YearMonth>="1月 1973", c("YearMonth", "decile", "Size_Ret")])

rm(size_DT)


##################################
#Decile Book-to-Market and Return#
##################################
...
#decile for BE/ME
BTM_DT[month(YearMonth)==7, 
       JulDecile := as.integer(
         cut(BTM[month(YearMonth)==7], 
             quantile(BTM[EXCHCD==1], 
                      probs = 0:10/10, 
                      na.rm = TRUE)), include.lowest = TRUE), by=.(YearMonth)]
...
BTM_Ret <- unique(BTM_DT[decile>0 & YearMonth>="1月 1973", c("YearMonth", "decile", "BTM_Ret")])
...
ff3$V1 <- NULL
ff3 <- ff3[YearMonth >= "1月 1973" & YearMonth <= "12月 2017"]
ff3[, RF:=RF/100]
...
BTM_DT <- BTM_DT[YearMonth >= "7月 1972"]

BTM_DT[month(YearMonth)==7, JulDecile := as.integer(
  cut(lag_Mkt_Cap, quantile(c(
    -Inf, lag_Mkt_Cap[
      EXCHCD==1], Inf), probs = 0:2/2, na.rm = TRUE)), 
  include.lowest = TRUE, include.highest = TRUE), by=.(YearMonth)]
...
doubleSort <- unique(BTM_DT[YearMonth >="1月 1973", c("YearMonth", "doubleSixSort", "doubleSortRet")])
...
########################################################
#do value and size anomaly worked in the past few years#
########################################################

#size anomaly
Portfolio_DT <- Portfolio_DT[, excessSizeRet:= Size_Ret-RF]
size_anomaly <- as.data.table(dcast(Portfolio_DT[, c(
  "YearMonth", "decile", "excessSizeRet")], YearMonth ~ decile, value.var = "excessSizeRet"))  
size_anomaly$smbExcess <- size_anomaly$`1`-size_anomaly$`10`

size_anomaly <- size_anomaly[YearMonth >= "1月 2008", c("YearMonth", "smbExcess")]
size_anomaly[, cumSMBRet:=cumprod(1+smbExcess)] 

#value anamoly
Portfolio_DT <- Portfolio_DT[, excessBTMRet:= BTM_Ret-RF]
value_anomaly <- as.data.table(dcast(Portfolio_DT[, c(
  "YearMonth", "decile", "excessBTMRet")], YearMonth ~ decile, value.var = "excessBTMRet"))  
value_anomaly$hmlExcess <- value_anomaly$`1`-value_anomaly$`10`

value_anomaly <- value_anomaly[YearMonth >= "1月 2008", c("YearMonth", "hmlExcess")]
value_anomaly[, cumHMLRet:=cumprod(1+hmlExcess)] 

size_value_anomaly <- merge(size_anomaly, value_anomaly, by=c("YearMonth"))
size_value_anomaly <- merge(size_value_anomaly, ff3[, c(
  "YearMonth", "Mkt-RF")], by.x = "YearMonth", by.y = "YearMonth", all.x = TRUE)
size_value_anomaly[, cumMktRet:=cumprod(1+`Mkt-RF`/100)]
#size_value_anomaly[, YearMonth:=as.integer(format(YearMonth, "%Y%m"))]

ggplot()+
  geom_line(data=size_value_anomaly,aes(x=size_value_anomaly$YearMonth,y= 
                                          size_value_anomaly$cumHMLRet, colour="H-L Excess Ret"), size=1)+
  geom_line(data=size_value_anomaly,aes(x=size_value_anomaly$YearMonth,y= 
                                          size_value_anomaly$cumSMBRet, colour="S-B Excess Ret"), size=1)+
  geom_line(data=size_value_anomaly,aes(x=size_value_anomaly$YearMonth,y= 
                                          size_value_anomaly$cumMktRet, colour="Market Excess Ret"), size=1)+
  geom_hline(yintercept=1, linetype="dotted", size=2)+
  scale_x_yearmon(size_value_anomaly$YearMonth, format = "%b %Y", n = 5)+
  labs(x = "Year Month",y="Cumulative Excess Long-Short Returns",colour="Labels",
       title="Size/Value Anamoly \n S-B, H-L decile cumulative returns")+
  theme(plot.title = element_text(color="Black", size=16, face="bold.italic", hjust=0.5),
        axis.title.x = element_text(color="blue", size=14, face="bold"),
        axis.title.y = element_text(color="#993333", size=14, face="bold"))

size_value_anomaly$YearMonth
...
doubleSort <- merge(doubleSort, ff3[, c(
  "YearMonth", "Mkt-RF")], by.x = "YearMonth", by.y = "YearMonth", all.x = TRUE)
doubleSort$`Mkt-RF` <- doubleSort$`Mkt-RF`/100
doubleSort <- doubleSort[YearMonth >= "1月 2008"]
...
ggplot()+
  ...
  labs(x = "Year Month",y="Cumulative Excess factor Return",colour="Labels",
       title="HML/SMB factors (Cumulative Return)")+
  theme(plot.title = element_text(color="Black", size=16, face="bold.italic", hjust=0.5),
        axis.title.x = element_text(color="blue", size=14, face="bold"),
        axis.title.y = element_text(color="#993333", size=14, face="bold"))
